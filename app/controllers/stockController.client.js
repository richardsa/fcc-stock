...
(function() {
  ...
  function getSymbols(data) {
    ...
    if (symbolsObject.length === 0) {
      symbolOutput += "<div class='alert alert-info'><h1 class='center-text'>Enter Stock Symbols Above</h1>";
      symbolOutput += "</div></div>";
  ...
  function displayStocks(data) {
    ...
    function drawChart() {
      ...
      // trigger socket reload after data is finished transferring/loading on client
      socket.emit('socket reload', symbols[i]);
    }
  ...
  $(document).ready(function() {
    ...
    $("#stockSymbols").on("click", ".btnDeleteStock", function(e) {
      ...
      socket.emit('socket reload', $(this).attr('id'));
    ...
    // reload across clients when triggered
    socket.on('socket reload', function(delSym) {
      ajaxFunctions.ajaxRequest('GET', appUrl + "/api/", getSymbols);
    });
  });
